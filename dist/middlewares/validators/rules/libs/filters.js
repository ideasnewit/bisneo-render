...
const pagination = [
    ...
    oneOf([
        check("limit")
            ...
            .escape()
            .isInt({ min: 5, max: 500 })
            .withMessage("Limit must be between 5 and 500")
            .toInt(),
        check("limit")
            .optional({ checkFalsy: true })
            .trim()
            .escape()
            .isAlpha()
            .withMessage("Limit must be alphabetic")
            .isIn(["all"])
            .withMessage("Invalid limit. Did you mean 'all'?")
            .toLowerCase(),
...
const sorting = [
    ...
    query("sort")
        ...
        .customSanitizer((sort) => sort.split(",").map((element) => element.trim().split(" ")))
        .custom((sort) => {
