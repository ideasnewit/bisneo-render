...
const { sequelize, Supplier, Purchase, Sale, SaleProduct, AmountReceived, Product, Category, Customer, User, Attendance, UserPayment, Client, } = db;
async function summary(req, res, next) {
    try {
        ...
        next({ status: 500, error: "Db error getting data" });
    }
}
    ...
    try {
        ...
        if (clientId) {
            const userCount = await User.count({
                where: { clientId: clientId.toString(), isActive: true },
            });
            const branchCount = await Client.count({
                where: { parentId: clientId.toString(), isActive: true },
            });
            return res.status(200).json({
                users: userCount,
...
export { summary, count, sales, productSales, stockCount, pendingBills, outOfStockProducts, topSellingProducts, topBuyingCustomers, topSellingUsers, topBuyingUsers, activeUsers, pendingTransactions, };
